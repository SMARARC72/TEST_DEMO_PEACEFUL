...
export function computeRiskPosture() {
  ...
  const tier = tierEl ? tierEl.value : 'T1';

  // T3 is always Elevated
  if (tier === 'T3') return 'Elevated';

  // Check triage queue for escalations
  ...
  // T2 with no escalations is Moderate
...
export function computePilotExpansionScore() {
  ...
  // Count unresolved ELEVATED triage items
