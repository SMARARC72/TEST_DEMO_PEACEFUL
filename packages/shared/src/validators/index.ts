...
const submissionStatusValues = ['PENDING', 'PROCESSING', 'READY', 'REVIEWED'] as const;
const triageStatusValues = ['ACK', 'IN_REVIEW', 'ESCALATED', 'RESOLVED'] as const;
const draftStatusValues = ['DRAFT', 'REVIEWED', 'APPROVED', 'REJECTED', 'ESCALATED'] as const;
...
export const patientSchema = z.object({
  ...
  diagnosis: z.object({
    ...
    code: z.string().regex(/^[A-Z]\d{2}(\.\d{1,4})?$/, 'Must be a valid ICD-10 code'),
  }),
