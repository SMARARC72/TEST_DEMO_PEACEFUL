...
describe('generateTokens', () => {
  ...
  it('PATIENT gets read:own and write:own', () => {
    const { accessToken } = generateTokens({ ...mockUser, role: UserRole.PATIENT });
    const decoded = jwt.verify(accessToken, JWT_SECRET) as Record<string, unknown>;
  ...
  it('ADMIN gets admin:all', () => {
    const { accessToken } = generateTokens({ ...mockUser, role: UserRole.ADMIN });
    const decoded = jwt.verify(accessToken, JWT_SECRET) as Record<string, unknown>;
    const perms = decoded.permissions as string[];
...
describe('verifyRefreshToken', () => {
  ...
  it('throws on wrong secret', () => {
    const badToken = jwt.sign({ sub: 'u1' }, 'wrong-secret', { expiresIn: '1h' });
