    ...
    $allModels: {
      async create({ model, args, query }) {
        if (args.data && PHI_FIELD_MAP[model]) {
          (args as { data: unknown }).data = encryptFields(model, args.data as Record<string, unknown>);
        }
      ...
      async createMany({ model, args, query }) {
        if (args.data && PHI_FIELD_MAP[model]) {
          if (Array.isArray(args.data)) {
            (args as { data: unknown }).data = (args.data as Record<string, unknown>[]).map((d) =>
              encryptFields(model, d),
            );
          } else {
            (args as { data: unknown }).data = encryptFields(model, args.data as Record<string, unknown>);
          }
      ...
      async upsert({ model, args, query }) {
        if (PHI_FIELD_MAP[model]) {
          if (args.create) {
            (args as { create: unknown }).create = encryptFields(model, args.create as Record<string, unknown>);
          }
          if (args.update) {
            (args as { update: unknown }).update = encryptFields(model, args.update as Record<string, unknown>);
          }
        }
        const result = await query(args);
        return decryptResult(model, result);
